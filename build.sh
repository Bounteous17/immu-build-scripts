...
make_packages() {
    # sed -i "s/#IgnorePkg   =/IgnorePkg = avahi/g" ${work_dir}/pacman.conf
    mkarchiso ${verbose} -w "${work_dir}/x86_64" -C "${work_dir}/pacman.conf" -D "${install_dir}" -P ${iso_publisher_author} -p "$(grep -h -v ^# ${script_path}/packages.x86_64)" install
}
...
run_once add_packages
run_once make_packages
run_once prepare_custom_packages
